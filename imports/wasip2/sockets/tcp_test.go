...
func TestTCPSocket_Connect(t *testing.T) {
	...
	result, err = tcpSocketRemoteAddress(ctx, []component.Val{clientBorrow})
	require.NoError(t, err)
...
func TestTCPSocket_Accept(t *testing.T) {
	...
	result, err = tcpSocketAccept(ctx, []component.Val{serverBorrow})
	require.NoError(t, err)
	isOk, tupleVal, _ := result[0].Result()
	...
	acceptedBorrow := component.ValBorrow(acceptedSocketHandle)

	// Verify the accepted socket has correct addresses
	result, err = tcpSocketLocalAddress(ctx, []component.Val{acceptedBorrow})
	require.NoError(t, err)
	isOk, _, _ = result[0].Result()
	require.True(t, isOk, "accepted socket local-address should succeed")

	result, err = tcpSocketRemoteAddress(ctx, []component.Val{acceptedBorrow})
	require.NoError(t, err)
	isOk, _, _ = result[0].Result()
...
func TestTCPSocket_IPv6(t *testing.T) {
	...
	result, err = tcpSocketAddressFamily(ctx, []component.Val{selfHandle})
	require.NoError(t, err)
	...
	addrRecord := component.ValRecord(map[string]component.Val{
		...
		"address": component.ValTuple([]component.Val{
			component.ValU16(0), component.ValU16(0), component.ValU16(0), component.ValU16(0),
			component.ValU16(0), component.ValU16(0), component.ValU16(0), component.ValU16(1),
		}),
	})
	localAddr := component.ValVariant("ipv6", &addrRecord)
