...
func resolveFuncType(ft *FuncType, localTypes map[uint32]*TypeDef) *FuncType {
	...
	for i, p := range ft.Params {
		...
		if td := getResolvedTypeDef(p.ValType, localTypes); td != nil {
			...
			resolved.Params[i].LocalTypes = localTypes
		}
	}
	...
	for i, r := range ft.Results {
		...
		if td := getResolvedTypeDef(r.ValType, localTypes); td != nil {
			...
			resolved.Results[i].LocalTypes = localTypes
		}
	}
...
func resolveToValType(nvt NamedValType, resolver *TypeResolver) types.ValType {
	...
	if nvt.ResolvedType != nil {
		return typeDefToValType(nvt.ResolvedType, nvt.LocalTypes)
	}
	...
	if nvt.ValType.IsPrimitive || nvt.ValType.IsOwn || nvt.ValType.IsBorrow {
		return valTypeRefToValType(nvt.ValType, nil)
	}
...
// localTypes provides the type context for resolving nested type index references.
// If the TypeDef has SourceLocalTypes set (from cross-instance resolution), those
// take priority over the passed-in localTypes since the TypeDef's internal
// ValTypeRef indices are relative to its source instance's type space.
func typeDefToValType(td *TypeDef, localTypes map[uint32]*TypeDef) types.ValType {
	if td == nil {
		return nil
	}
	// Use the TypeDef's source local types if available, as the internal
	// type references are relative to the source instance's type space
	if td.SourceLocalTypes != nil {
		localTypes = td.SourceLocalTypes
	}
	...
	case TypeDefKindDefined:
		...
		if td.Handle != nil {
			if td.Handle.IsPrimitive {
				...
				return valTypeRefToValType(*td.Handle, localTypes)
			}
			if td.Handle.IsOwn {
		...
		if td.Option != nil {
			inner := valTypeRefToValType(td.Option.InnerType, localTypes)
			return types.Option{Some: inner}
		}
		if td.Result != nil {
			var okType, errType types.ValType
			if td.Result.OkType != nil {
				okType = valTypeRefToValType(*td.Result.OkType, localTypes)
			}
			if td.Result.ErrType != nil {
				errType = valTypeRefToValType(*td.Result.ErrType, localTypes)
			}
		...
		if td.Record != nil {
			...
			for i, f := range td.Record.Fields {
				fields[i] = types.Field{
					Name: f.Name,
					Type: valTypeRefToValType(f.ValType, localTypes),
				}
			}
		...
		if td.Tuple != nil {
			...
			for i, t := range td.Tuple.Types {
				elemTypes[i] = valTypeRefToValType(t, localTypes)
			}
			return types.Tuple{Types: elemTypes}
		}
		if td.List != nil {
			return types.List{Element: valTypeRefToValType(td.List.ElementType, localTypes)}
		}
		if td.Variant != nil {
			...
			for i, c := range td.Variant.Cases {
				...
				if c.ValType != nil {
					caseType = valTypeRefToValType(*c.ValType, localTypes)
				}
...
func valTypeRefToValType(ref ValTypeRef, localTypes map[uint32]*TypeDef) types.ValType {
	if ref.IsPrimitive {
	...
	// For type index references, resolve using the local type context
	if localTypes != nil {
		if td, ok := localTypes[ref.TypeIdx]; ok {
			if vt := typeDefToValType(td, localTypes); vt != nil {
				return vt
			}
		}
	}

	// Fallback: can't resolve without context
	return types.U32{}
}
	...
	return api.GoModuleFunc(func(ctx context.Context, mod api.Module, stack []uint64) {
		...
		if compFunc.Type != nil {
			for _, paramDef := range compFunc.Type.Params {
				...
				if localTypes == nil && paramDef.ResolvedType != nil {
					localTypes = paramDef.ResolvedType.SourceLocalTypes
				}
				val, consumed := liftFromStack(stack[stackIdx:], paramDef.ValType, paramDef.ResolvedType, localTypes, memory)
		...
		if needsRetptr && memory != nil {
			...
			if err := writeResultsToMemory(ctx, memory, reallocFunc, retptr, results, compFunc.Type); err != nil {
				...
				// just sees uninitialized memory at retptr.
			...
			for i, result := range results {
				var resultResolved *TypeDef
				if compFunc.Type != nil && i < len(compFunc.Type.Results) {
					resultResolved = compFunc.Type.Results[i].ResolvedType
				}
				written := lowerToStack(stack[resultIdx:], result, resultResolved)
				resultIdx += written
			}
		}
	})
...
func liftFromStack(stack []uint64, typeRef ValTypeRef, resolvedType *TypeDef, localTypes map[uint32]*TypeDef, memory api.Memory) (Val, int) {
	...
	if resolvedType != nil && resolvedType.Kind == TypeDefKindDefined {
		...
		// Handle record types
		if resolvedType.Record != nil {
			// Prefer SourceLocalTypes from the resolved type itself, as it
			// represents the correct type scope for the record's fields.
			// The outer localTypes may map the same TypeIdx to different types.
			effectiveLocalTypes := resolvedType.SourceLocalTypes
			if effectiveLocalTypes == nil {
				effectiveLocalTypes = localTypes
			}
			return liftRecordFromStack(stack, resolvedType.Record, effectiveLocalTypes, memory)
		}

		// Handle option types
		if resolvedType.Option != nil {
			effectiveLocalTypes := resolvedType.SourceLocalTypes
			if effectiveLocalTypes == nil {
				effectiveLocalTypes = localTypes
			}
			return liftOptionFromStack(stack, resolvedType.Option, effectiveLocalTypes, memory)
		}

		// Handle variant types
		if resolvedType.Variant != nil {
			effectiveLocalTypes := resolvedType.SourceLocalTypes
			if effectiveLocalTypes == nil {
				effectiveLocalTypes = localTypes
			}
			return liftVariantFromStack(stack, resolvedType.Variant, effectiveLocalTypes, memory)
		}
...
func liftRecordFromStack(stack []uint64, recordDef *RecordTypeDef, localTypes map[uint32]*TypeDef, memory api.Memory) (Val, int) {
	...
	for _, field := range recordDef.Fields {
		...
		val, consumed := liftFromStack(stack[stackIdx:], field.ValType, fieldResolved, localTypes, memory)
...
func liftListFromMemory(ptr, length uint32, elemType ValTypeRef, elemResolved *TypeDef, localTypes map[uint32]*TypeDef, memory api.Memory) []Val {
	...
	elemSize := elemSizeFromTypeDef(elemResolved)
	if elemSize == 0 {
		return nil
	}
...
func liftValFromMemory(offset uint32, typeRef ValTypeRef, resolved *TypeDef, localTypes map[uint32]*TypeDef, memory api.Memory) Val {
	...
	if resolved.Record != nil {
		...
		return liftRecordFromMemory(offset, resolved.Record, effectiveLocalTypes, memory)
	}
	if resolved.Option != nil {
		effectiveLocalTypes := resolved.SourceLocalTypes
		if effectiveLocalTypes == nil {
			effectiveLocalTypes = localTypes
		}
		return liftOptionFromMemory(offset, resolved.Option, effectiveLocalTypes, memory)
	}
	if resolved.List != nil {
		ptrVal, _ := memory.ReadUint32Le(offset)
		lenVal, _ := memory.ReadUint32Le(offset + 4)
...
func liftRecordFromMemory(offset uint32, recordDef *RecordTypeDef, localTypes map[uint32]*TypeDef, memory api.Memory) Val {
	...
	for _, field := range recordDef.Fields {
		fieldResolved := resolveInnerType(field.ValType, localTypes)
		fieldSize := elemSizeFromTypeRef(field.ValType, fieldResolved)
		fieldAlign := elemAlignFromTypeRef(field.ValType, fieldResolved)
		// Align field offset
		if fieldAlign > 0 {
...
func elemSizeFromTypeDef(td *TypeDef) uint32 {
	if td.Enum != nil {
		if len(td.Enum.Names) <= 256 {
			return 1
		}
		return 4
	}
	if td.Record != nil {
		return recordSize(td.Record, td.SourceLocalTypes)
	}
	if td.Option != nil {
		return optionSize(td.Option, td.SourceLocalTypes)
	}
	if td.List != nil {
		return 8 // ptr + len
	}
	if td.Flags != nil {
		n := len(td.Flags.Names)
		return uint32((n + 31) / 32 * 4)
	}
	return 4
}

// elemSizeFromTypeRef returns the byte size of a ValTypeRef.
func elemSizeFromTypeRef(ref ValTypeRef, resolved *TypeDef) uint32 {
	if ref.IsPrimitive {
		switch ref.Primitive {
		case 0x7f, 0x7e, 0x7d: // bool, s8, u8
			return 1
		case 0x7c, 0x7b: // s16, u16
			return 2
		case 0x78, 0x77: // s64, u64
			return 8
		case 0x76: // f32
			return 4
		case 0x75: // f64
	...
	if resolved != nil {
		return elemSizeFromTypeDef(resolved)
	}
	return 4
}

// elemAlignFromTypeRef returns the alignment of a ValTypeRef.
func elemAlignFromTypeRef(ref ValTypeRef, resolved *TypeDef) uint32 {
	if ref.IsPrimitive {
		switch ref.Primitive {
		case 0x7f, 0x7e, 0x7d: // bool, s8, u8
			return 1
		case 0x7c, 0x7b: // s16, u16
			return 2
		case 0x78, 0x77, 0x75: // s64, u64, f64
			return 8
		case 0x73: // string
			return 4 // ptr alignment
		default:
			return 4
		}
	}
	if resolved != nil {
		return elemAlignFromTypeDef(resolved)
	}
	return 4
}

// elemAlignFromTypeDef returns the alignment of a TypeDef.
func elemAlignFromTypeDef(td *TypeDef) uint32 {
	if td.Enum != nil {
		return 1
	}
	if td.Record != nil {
		return recordAlign(td.Record, td.SourceLocalTypes)
	}
	if td.Option != nil {
		return optionAlign(td.Option, td.SourceLocalTypes)
	}
	if td.List != nil {
		return 4 // ptr alignment
	}
...
// recordSize returns the total size of a record in memory.
func recordSize(rec *RecordTypeDef, localTypes map[uint32]*TypeDef) uint32 {
	var offset uint32
	maxAlign := uint32(1)
...
// recordAlign returns the alignment of a record.
func recordAlign(rec *RecordTypeDef, localTypes map[uint32]*TypeDef) uint32 {
	maxAlign := uint32(1)
	for _, field := range rec.Fields {
		fieldResolved := resolveInnerType(field.ValType, localTypes)
		fieldAlign := elemAlignFromTypeRef(field.ValType, fieldResolved)
...
// optionSize returns the size of an option in memory.
func optionSize(opt *OptionTypeDef, localTypes map[uint32]*TypeDef) uint32 {
	innerResolved := resolveInnerType(opt.InnerType, localTypes)
	innerSize := elemSizeFromTypeRef(opt.InnerType, innerResolved)
	innerAlign := elemAlignFromTypeRef(opt.InnerType, innerResolved)
	// discriminant (1 byte) + padding + payload
	payloadOffset := (uint32(1) + innerAlign - 1) &^ (innerAlign - 1)
	total := payloadOffset + innerSize
	// Align to max(1, innerAlign)
	align := innerAlign
	if align < 1 {
		align = 1
	}
...
// optionAlign returns the alignment of an option.
func optionAlign(opt *OptionTypeDef, localTypes map[uint32]*TypeDef) uint32 {
	innerResolved := resolveInnerType(opt.InnerType, localTypes)
	innerAlign := elemAlignFromTypeRef(opt.InnerType, innerResolved)
	if innerAlign < 1 {
		return 1
...
// resolveInnerType resolves a ValTypeRef to its TypeDef using localTypes.
// It also handles nested references where the type at TypeIdx itself references another type
// through SourceLocalTypes.
func resolveInnerType(ref ValTypeRef, localTypes map[uint32]*TypeDef) *TypeDef {
	if ref.IsPrimitive || ref.IsOwn || ref.IsBorrow || localTypes == nil {
		return nil
	}
	td := localTypes[ref.TypeIdx]
	if td != nil {
		// If this TypeDef has its own SourceLocalTypes, propagate them
		return td
	}
	return nil
}
...
func liftVariantFromStack(stack []uint64, variantDef *VariantTypeDef, localTypes map[uint32]*TypeDef, memory api.Memory) (Val, int) {
	...
	for _, c := range variantDef.Cases {
		if c.ValType != nil {
			slots := flatSlotCount(*c.ValType, localTypes)
			if slots > maxPayloadSlots {
				maxPayloadSlots = slots
			}
		}
	}
...
func flatSlotCount(ref ValTypeRef, localTypes map[uint32]*TypeDef) int {
	if ref.IsPrimitive {
		switch ref.Primitive {
		case 0x73: // string
			return 2
	...
	if td.Option != nil {
		return 1 + flatSlotCount(td.Option.InnerType, localTypes)
	}
	if td.Record != nil {
		total := 0
		for _, f := range td.Record.Fields {
			total += flatSlotCount(f.ValType, localTypes)
		}
		return total
	}
	if td.Variant != nil {
		maxPayload := 0
		for _, c := range td.Variant.Cases {
			if c.ValType != nil {
				s := flatSlotCount(*c.ValType, localTypes)
				if s > maxPayload {
					maxPayload = s
				}
			}
...
// ============================================================================
// Canonical ABI alignment / element-size helpers
//
// These functions implement the alignment(t) and elem_size(t) definitions
// from the Component Model Canonical ABI specification (CanonicalABI.md
// sections "Alignment" and "Element Size"). They are type-aware: callers
// pass a ValTypeRef plus the localTypes map that resolves type indices to
// TypeDefs. This is the foundation for spec-conformant lowering of
// composite types (list, record, tuple, variant, option, result, ...).
//
// Per the spec:
//   alignment(t) gives the alignment in bytes of t.
//   elem_size(t) gives the storage size in bytes of t when used as an
//                element of a list (or field of a record); this includes
//                trailing padding for record/tuple types.
//
// Both functions despecialize tuple into record and option/result into
// variant before computing.
// ============================================================================

// cabiAlignTypeRef returns alignment(t) per the canonical ABI for a ValTypeRef.
func cabiAlignTypeRef(ref ValTypeRef, localTypes map[uint32]*TypeDef) uint32 {
	return cabiAlignTypeRefSeen(ref, localTypes, nil)
}

// cabiAlignTypeRefSeen is the recursive helper that tracks visited TypeIdx
// values to break cycles. The Component Model spec disallows cyclic value
// types, but the wazero type-resolution layer can produce cyclic graphs in
// some edge cases (e.g., when an instance type's local type space includes
// shadowing aliases). Cycle detection prevents stack-overflow regressions.
func cabiAlignTypeRefSeen(ref ValTypeRef, localTypes map[uint32]*TypeDef, seen map[uint32]bool) uint32 {
	if ref.IsPrimitive {
		switch ref.Primitive {
		case 0x7f, 0x7e, 0x7d: // bool, s8, u8
			return 1
		case 0x7c, 0x7b: // s16, u16
			return 2
		case 0x7a, 0x79, 0x76, 0x74, 0x73: // s32, u32, f32, char, string
			return 4
		case 0x78, 0x77, 0x75: // s64, u64, f64
			return 8
		}
	...
	if seen[ref.TypeIdx] {
		// Cycle detected; conservatively use pointer alignment.
		return 4
	}
	td := localTypes[ref.TypeIdx]
	if td == nil {
	...
	seen[ref.TypeIdx] = true
	defer delete(seen, ref.TypeIdx)
	return cabiAlignTypeDefSeen(td, localTypes, seen)
}

// cabiAlignTypeDef returns alignment(t) per the canonical ABI for a TypeDef.
// localTypes is used to resolve nested type references.
func cabiAlignTypeDef(td *TypeDef, localTypes map[uint32]*TypeDef) uint32 {
	return cabiAlignTypeDefSeen(td, localTypes, nil)
}

// cabiAlignTypeDefSeen is the recursive helper that tracks visited TypeIdx
// values to break cycles.
func cabiAlignTypeDefSeen(td *TypeDef, localTypes map[uint32]*TypeDef, seen map[uint32]bool) uint32 {
	if td == nil {
		return 4
	}
	// The Handle field is overloaded by the binary decoder to represent
	// both actual own/borrow handle types AND primitive-type aliases (e.g.
	// when an instance type declares a top-level `(type 12 u64)` that
	// becomes a TypeDef with Kind=Defined and Handle=&{IsPrimitive,
	// Primitive:0x77}). Recurse into the underlying ValTypeRef so that
	// alignment is computed from the despecialized type.
	if td.Handle != nil {
		return cabiAlignTypeRefSeen(*td.Handle, localTypes, seen)
	}
	if td.Enum != nil {
		// Enum despecializes to a variant of all-empty cases. Discriminant
		// is the smallest int that fits len(cases).
		n := len(td.Enum.Names)
		switch {
		case n <= 0x100:
			return 1
	...
	if td.List != nil {
		// Variable-length list lowers as (ptr, len), aligned at 4.
		return 4
	}
	if td.FixedSizeList != nil {
		return cabiAlignTypeRefSeen(td.FixedSizeList.ElementType, localTypes, seen)
	}
	if td.Record != nil {
		a := uint32(1)
		for _, f := range td.Record.Fields {
			fa := cabiAlignTypeRefSeen(f.ValType, localTypes, seen)
			if fa > a {
				a = fa
			}
		}
	...
	if td.Tuple != nil {
		// Tuple despecializes to record.
		a := uint32(1)
		for _, t := range td.Tuple.Types {
			fa := cabiAlignTypeRefSeen(t, localTypes, seen)
			if fa > a {
				a = fa
			}
		}
	...
	if td.Variant != nil {
		return cabiVariantAlignSeen(td.Variant.Cases, localTypes, seen)
	}
	if td.Option != nil {
		// Option despecializes to variant { none, some(T) }.
		// Discriminant size for 2 cases is 1 byte (alignment 1).
		// Max case alignment is alignment(InnerType).
		ia := cabiAlignTypeRefSeen(td.Option.InnerType, localTypes, seen)
		if ia > 1 {
			return ia
		}
		return 1
	...
	if td.Result != nil {
		// Result despecializes to variant { ok, err }.
		// Discriminant size for 2 cases is 1 byte.
		var a uint32 = 1
		if td.Result.OkType != nil {
			if oa := cabiAlignTypeRefSeen(*td.Result.OkType, localTypes, seen); oa > a {
				a = oa
			}
		}
		if td.Result.ErrType != nil {
			if ea := cabiAlignTypeRefSeen(*td.Result.ErrType, localTypes, seen); ea > a {
				a = ea
			}
		}
		return a
	}
	if td.Stream != nil || td.Future != nil {
		return 4
	}
	return 4
}

// cabiVariantAlign returns alignment_variant(cases).
func cabiVariantAlign(cases []VariantCase, localTypes map[uint32]*TypeDef) uint32 {
	return cabiVariantAlignSeen(cases, localTypes, nil)
}

func cabiVariantAlignSeen(cases []VariantCase, localTypes map[uint32]*TypeDef, seen map[uint32]bool) uint32 {
	// Discriminant alignment based on case count.
	var discAlign uint32 = 1
	n := len(cases)
	switch {
	case n <= 0x100:
		discAlign = 1
	...
	maxCaseAlign := uint32(1)
	for _, c := range cases {
		if c.ValType != nil {
			ca := cabiAlignTypeRefSeen(*c.ValType, localTypes, seen)
			if ca > maxCaseAlign {
				maxCaseAlign = ca
			}
		}
	}
	if discAlign > maxCaseAlign {
		return discAlign
	}
	return maxCaseAlign
}

// cabiSizeTypeRef returns elem_size(t) per the canonical ABI for a ValTypeRef.
func cabiSizeTypeRef(ref ValTypeRef, localTypes map[uint32]*TypeDef) uint32 {
	return cabiSizeTypeRefSeen(ref, localTypes, nil)
}

func cabiSizeTypeRefSeen(ref ValTypeRef, localTypes map[uint32]*TypeDef, seen map[uint32]bool) uint32 {
	if ref.IsPrimitive {
		switch ref.Primitive {
		case 0x7f, 0x7e, 0x7d: // bool, s8, u8
			return 1
		case 0x7c, 0x7b: // s16, u16
			return 2
		case 0x7a, 0x79, 0x76, 0x74: // s32, u32, f32, char
			return 4
		case 0x78, 0x77, 0x75: // s64, u64, f64
			return 8
		case 0x73: // string: (ptr, len) pair
			return 8
		}
		return 4
	}
	...
	seen[ref.TypeIdx] = true
	defer delete(seen, ref.TypeIdx)
	return cabiSizeTypeDefSeen(td, localTypes, seen)
}

// cabiSizeTypeDef returns elem_size(t) per the canonical ABI for a TypeDef.
func cabiSizeTypeDef(td *TypeDef, localTypes map[uint32]*TypeDef) uint32 {
	return cabiSizeTypeDefSeen(td, localTypes, nil)
}

func cabiSizeTypeDefSeen(td *TypeDef, localTypes map[uint32]*TypeDef, seen map[uint32]bool) uint32 {
	if td == nil {
		return 4
	}
	// The Handle field is overloaded by the binary decoder to represent
	// both actual own/borrow handle types AND primitive-type aliases.
	// Recurse into the underlying ValTypeRef so that the size is computed
	// from the despecialized type.
	if td.Handle != nil {
		return cabiSizeTypeRefSeen(*td.Handle, localTypes, seen)
	}
	if td.Enum != nil {
		n := len(td.Enum.Names)
		switch {
	...
	if td.List != nil {
		// Variable-length list: (ptr, len), 8 bytes total.
		return 8
	}
	if td.FixedSizeList != nil {
		es := cabiSizeTypeRefSeen(td.FixedSizeList.ElementType, localTypes, seen)
		return td.FixedSizeList.Size * es
	}
	if td.Record != nil {
		return cabiRecordSizeSeen(td.Record.Fields, localTypes, seen)
	}
	if td.Tuple != nil {
		// Tuple despecializes to record.
		return cabiTupleSizeSeen(td.Tuple.Types, localTypes, seen)
	}
	if td.Variant != nil {
		return cabiVariantSizeSeen(td.Variant.Cases, localTypes, seen)
	}
	if td.Option != nil {
		return cabiOptionSizeSeen(td.Option.InnerType, localTypes, seen)
	}
	if td.Result != nil {
		return cabiResultSizeSeen(td.Result, localTypes, seen)
	}
	if td.Stream != nil || td.Future != nil {
		return 4
	}
...
// cabiRecordSize returns elem_size_record(fields) per the canonical ABI.
func cabiRecordSize(fields []RecordField, localTypes map[uint32]*TypeDef) uint32 {
	return cabiRecordSizeSeen(fields, localTypes, nil)
}

func cabiRecordSizeSeen(fields []RecordField, localTypes map[uint32]*TypeDef, seen map[uint32]bool) uint32 {
	var s uint32 = 0
	maxAlign := uint32(1)
	for _, f := range fields {
		...
		s = alignTo(s, fa)
		s += cabiSizeTypeRefSeen(f.ValType, localTypes, seen)
	}
	return alignTo(s, maxAlign)
}

// cabiTupleSize is the same as cabiRecordSize but for tuple types.
func cabiTupleSize(types []ValTypeRef, localTypes map[uint32]*TypeDef) uint32 {
	return cabiTupleSizeSeen(types, localTypes, nil)
}

func cabiTupleSizeSeen(types []ValTypeRef, localTypes map[uint32]*TypeDef, seen map[uint32]bool) uint32 {
	var s uint32 = 0
	maxAlign := uint32(1)
	for _, t := range types {
		fa := cabiAlignTypeRefSeen(t, localTypes, seen)
		...
		s = alignTo(s, fa)
		s += cabiSizeTypeRefSeen(t, localTypes, seen)
	}
	return alignTo(s, maxAlign)
}

// cabiVariantSize returns elem_size_variant(cases) per the canonical ABI.
func cabiVariantSize(cases []VariantCase, localTypes map[uint32]*TypeDef) uint32 {
	return cabiVariantSizeSeen(cases, localTypes, nil)
}

func cabiVariantSizeSeen(cases []VariantCase, localTypes map[uint32]*TypeDef, seen map[uint32]bool) uint32 {
	// Discriminant size.
	var discSize uint32
	n := len(cases)
	switch {
	...
	// Max case alignment determines payload alignment.
	maxCaseAlign := uint32(1)
	...
	for _, c := range cases {
		if c.ValType != nil {
			...
			cs := cabiSizeTypeRefSeen(*c.ValType, localTypes, seen)
			if cs > maxCaseSize {
				maxCaseSize = cs
			}
		}
	}
	s := alignTo(discSize, maxCaseAlign)
	s += maxCaseSize
	// Pad to alignment_variant.
	va := discSize
	if maxCaseAlign > va {
		va = maxCaseAlign
	}
...
// cabiOptionSize returns elem_size for an option type, despecialized as a variant.
func cabiOptionSize(inner ValTypeRef, localTypes map[uint32]*TypeDef) uint32 {
	return cabiOptionSizeSeen(inner, localTypes, nil)
}

func cabiOptionSizeSeen(inner ValTypeRef, localTypes map[uint32]*TypeDef, seen map[uint32]bool) uint32 {
	cases := []VariantCase{
		{Name: "none", ValType: nil},
		{Name: "some", ValType: &inner},
	}
	return cabiVariantSizeSeen(cases, localTypes, seen)
}

// cabiResultSize returns elem_size for a result type, despecialized as a variant.
func cabiResultSize(rdef *ResultTypeDef, localTypes map[uint32]*TypeDef) uint32 {
	return cabiResultSizeSeen(rdef, localTypes, nil)
}

func cabiResultSizeSeen(rdef *ResultTypeDef, localTypes map[uint32]*TypeDef, seen map[uint32]bool) uint32 {
	cases := []VariantCase{
		{Name: "ok", ValType: rdef.OkType},
		{Name: "err", ValType: rdef.ErrType},
	}
	return cabiVariantSizeSeen(cases, localTypes, seen)
}

// canWriteValTyped returns true if writeValTyped has enough type information
// to handle a value of the given type. This is the case when the type is a
// primitive, an own/borrow handle, or a TypeDef that resolves through the
// localTypes map.
func canWriteValTyped(ref ValTypeRef, localTypes map[uint32]*TypeDef) bool {
	if ref.IsPrimitive {
		return true
	}
	if ref.IsOwn || ref.IsBorrow {
...
// writeValTyped writes a single component value into linear memory at the
// given offset, using full type information (the value's expected ValTypeRef
// plus the localTypes map for resolving type indices). It implements the
// canonical ABI store(cx, v, t, ptr) function for the cases that wazero
// currently supports.
//
// The caller is responsible for ensuring that offset is already aligned to
// alignment(typeRef), as required by the spec.
//
// On success, returns nil. The number of bytes written equals
// cabiSizeTypeRef(typeRef, localTypes); the offset advance must be done by
// the caller.
func writeValTyped(
	ctx context.Context,
	memory api.Memory,
	...
	// Resolve typeRef to a TypeDef when applicable so we can dispatch on the
	// despecialized type rather than the runtime kind. This is important
	// because the runtime Val may carry slightly different shapes (e.g. a
	// tuple represented via Val.Tuple() vs. a record represented via
	// Val.Record()).
	if typeRef.IsPrimitive {
		// Strings are special: they require realloc to allocate the byte
		// buffer, then write (ptr, len). Handle that here rather than in
		// writeValPrimitive (which has no realloc access).
		if typeRef.Primitive == 0x73 { // string
			return writeStringTyped(ctx, memory, reallocFunc, offset, val.StringVal())
		}
		return writeValPrimitive(memory, offset, val, typeRef.Primitive)
	}
	if typeRef.IsOwn || typeRef.IsBorrow {
		// Per the spec, own and borrow handles are stored as 4-byte
		// indices. The host has already placed the resource into the
		// destination component instance's table; the index it returned
		...
		case ValKindS32, ValKindU32:
			idx = val.U32()
		default:
			return fmt.Errorf("writeValTyped: expected own/borrow handle, got %s", val.Kind())
		}
		if !memory.WriteUint32Le(offset, idx) {
			return fmt.Errorf("writeValTyped: failed to write own/borrow handle at offset %d", offset)
		}
		return nil
	}

	td := localTypes[typeRef.TypeIdx]
	if td == nil {
		return fmt.Errorf("writeValTyped: unresolved type index %d", typeRef.TypeIdx)
	}
	// When descending into a TypeDef whose internal type indices come
	// from a different scope (e.g. a record imported via outer alias),
	// switch to the TypeDef's own SourceLocalTypes for any nested
	// references. This matches the convention used by the lift code
	// (liftRecordFromMemory etc.) and is required for shared types.
	if len(td.SourceLocalTypes) > 0 {
		localTypes = td.SourceLocalTypes
	}
	// Handle types stored as TypeDefs (e.g., own<T>/borrow<T> referenced
	// indirectly through a type alias).
	if td.Handle != nil {
		return writeValTyped(ctx, memory, reallocFunc, offset, val, *td.Handle, localTypes)
	}
	if td.Enum != nil {
		// Look up discriminant index from the enum case name.
		caseName := val.Enum()
		var disc uint32
		for i, name := range td.Enum.Names {
			if name == caseName {
				disc = uint32(i)
				break
			}
		}
		// Write disc as the smallest integer covering len(cases).
		size := cabiSizeTypeDef(td, localTypes)
		switch size {
		case 1:
			if !memory.WriteByteAt(offset, byte(disc)) {
				return fmt.Errorf("writeValTyped: failed to write enum disc")
			}
		case 2:
			if !memory.WriteUint16Le(offset, uint16(disc)) {
				return fmt.Errorf("writeValTyped: failed to write enum disc")
			}
		default:
			if !memory.WriteUint32Le(offset, disc) {
				return fmt.Errorf("writeValTyped: failed to write enum disc")
			}
		}
		return nil
	...
	if td.Flags != nil {
		flags := val.Flags()
		var bits uint32
		for i, name := range td.Flags.Names {
			if flags[name] {
				bits |= 1 << uint(i)
			}
		}
		...
		switch size {
		case 1:
			if !memory.WriteByteAt(offset, byte(bits)) {
				return fmt.Errorf("writeValTyped: failed to write flags")
			}
		case 2:
			if !memory.WriteUint16Le(offset, uint16(bits)) {
				return fmt.Errorf("writeValTyped: failed to write flags")
			}
		default:
			if !memory.WriteUint32Le(offset, bits) {
				return fmt.Errorf("writeValTyped: failed to write flags")
			}
		}
		return nil
	...
	if td.Record != nil {
		// Despecialized: store each field at its aligned offset.
		// Accept records produced as either ValKindRecord (a map) or
		// ValKindTuple (a positional slice). The latter is occasionally
		// used by host code that builds records positionally.
		var rec map[string]Val
		switch val.Kind() {
		case ValKindRecord:
			rec = val.Record()
		case ValKindTuple:
			elems := val.Tuple()
			if len(elems) != len(td.Record.Fields) {
				return fmt.Errorf("writeValTyped: record arity mismatch: expected %d fields, got tuple of %d", len(td.Record.Fields), len(elems))
			}
			rec = make(map[string]Val, len(elems))
			for i, f := range td.Record.Fields {
				rec[f.Name] = elems[i]
			}
		default:
			return fmt.Errorf("writeValTyped: expected record value, got %s", val.Kind())
		}
		fieldOffset := offset
		for _, f := range td.Record.Fields {
			fieldAlign := cabiAlignTypeRef(f.ValType, localTypes)
			fieldOffset = alignTo(fieldOffset, fieldAlign)
			fv, ok := rec[f.Name]
			if !ok {
				return fmt.Errorf("writeValTyped: record field %q missing", f.Name)
			}
			if err := writeValTyped(ctx, memory, reallocFunc, fieldOffset, fv, f.ValType, localTypes); err != nil {
				return fmt.Errorf("writeValTyped: record field %q: %w", f.Name, err)
			}
			fieldOffset += cabiSizeTypeRef(f.ValType, localTypes)
		}
		return nil
	}
	if td.Tuple != nil {
		// Despecialized: store each element at its aligned offset.
		// Accept either a Val of kind ValKindTuple or ValKindRecord (in case
		// the host built it as a record-like value).
		var elems []Val
		switch val.Kind() {
		case ValKindTuple:
			elems = val.Tuple()
		case ValKindList:
			// Some host code may use a list for the tuple shape.
			elems = val.List()
		default:
			return fmt.Errorf("writeValTyped: expected tuple value, got %s", val.Kind())
		}
		if len(elems) != len(td.Tuple.Types) {
			return fmt.Errorf("writeValTyped: tuple arity mismatch: expected %d, got %d", len(td.Tuple.Types), len(elems))
		}
		fieldOffset := offset
		for i, t := range td.Tuple.Types {
			fa := cabiAlignTypeRef(t, localTypes)
			fieldOffset = alignTo(fieldOffset, fa)
			if err := writeValTyped(ctx, memory, reallocFunc, fieldOffset, elems[i], t, localTypes); err != nil {
				return fmt.Errorf("writeValTyped: tuple element %d: %w", i, err)
			}
			fieldOffset += cabiSizeTypeRef(t, localTypes)
		}
		return nil
	}
	if td.List != nil {
		// store_list: allocate via realloc, write elements, write (ptr, len).
		list := val.List()
		listLen := uint32(len(list))
		elemRef := td.List.ElementType
		elemSize := cabiSizeTypeRef(elemRef, localTypes)
		elemAlign := cabiAlignTypeRef(elemRef, localTypes)
		var basePtr uint32
		if listLen > 0 {
			if reallocFunc == nil {
				return fmt.Errorf("writeValTyped: list lowering requires realloc")
			}
			byteLen := uint64(listLen) * uint64(elemSize)
			results, err := reallocFunc.Call(ctx, 0, 0, uint64(elemAlign), byteLen)
			if err != nil {
				return fmt.Errorf("writeValTyped: realloc for list failed: %w", err)
			}
			basePtr = uint32(results[0])
			for i, elem := range list {
				elemOffset := basePtr + uint32(i)*elemSize
				if err := writeValTyped(ctx, memory, reallocFunc, elemOffset, elem, elemRef, localTypes); err != nil {
					return fmt.Errorf("writeValTyped: list element %d: %w", i, err)
				}
			}
		}
		if !memory.WriteUint32Le(offset, basePtr) {
			return fmt.Errorf("writeValTyped: failed to write list ptr")
		}
		if !memory.WriteUint32Le(offset+4, listLen) {
			return fmt.Errorf("writeValTyped: failed to write list len")
		}
		return nil
	}
	if td.Option != nil {
		// Option despecializes to variant { none, some(T) }. Per the
		// canonical ABI store_variant, the payload offset uses
		// max_case_alignment(cases), which for option is alignment(T).
		payload := val.Option()
		innerRef := td.Option.InnerType
		innerAlign := cabiAlignTypeRef(innerRef, localTypes)
		// disc_size for 2 cases = 1 byte
		disc := byte(0)
		if payload != nil {
			disc = 1
		}
		if !memory.WriteByteAt(offset, disc) {
			return fmt.Errorf("writeValTyped: failed to write option disc")
		}
		if payload == nil {
			// The payload bytes are uninitialized but the canonical ABI
			// reserves space; the guest must not read them when disc=0.
			return nil
		}
		// Payload starts at align_to(offset+disc_size, max_case_alignment).
		payloadOffset := alignTo(offset+1, innerAlign)
		return writeValTyped(ctx, memory, reallocFunc, payloadOffset, *payload, innerRef, localTypes)
	}
	if td.Result != nil {
		// Result despecializes to variant { ok(T)?, err(E)? }. Per the
		// canonical ABI store_variant, the payload offset uses
		// max_case_alignment(cases), NOT just the alignment of the
		// selected case's type.
		isOk, okVal, errVal := val.Result()
		var disc byte
		var payload *Val
		var payloadRef *ValTypeRef
		if isOk {
			disc = 0
			payload = okVal
			payloadRef = td.Result.OkType
		} else {
			disc = 1
			payload = errVal
			payloadRef = td.Result.ErrType
		}
		if !memory.WriteByteAt(offset, disc) {
			return fmt.Errorf("writeValTyped: failed to write result disc")
		}
		// Compute max_case_alignment across both ok and err.
		maxCaseAlign := uint32(1)
		if td.Result.OkType != nil {
			if a := cabiAlignTypeRef(*td.Result.OkType, localTypes); a > maxCaseAlign {
				maxCaseAlign = a
			}
		}
		if td.Result.ErrType != nil {
			if a := cabiAlignTypeRef(*td.Result.ErrType, localTypes); a > maxCaseAlign {
				maxCaseAlign = a
			}
		}
		if payload == nil || payloadRef == nil {
			return nil
		}
		payloadOffset := alignTo(offset+1, maxCaseAlign)
		return writeValTyped(ctx, memory, reallocFunc, payloadOffset, *payload, *payloadRef, localTypes)
	}
	if td.Variant != nil {
		...
		if caseIdx < 0 {
			return fmt.Errorf("writeValTyped: unknown variant case %q", caseName)
		}
		discSize := uint32(1)
		n := len(td.Variant.Cases)
		switch {
		case n <= 0x100:
			discSize = 1
			if !memory.WriteByteAt(offset, byte(caseIdx)) {
				return fmt.Errorf("writeValTyped: failed to write variant disc")
			}
		case n <= 0x10000:
			discSize = 2
			if !memory.WriteUint16Le(offset, uint16(caseIdx)) {
				return fmt.Errorf("writeValTyped: failed to write variant disc")
			}
		default:
			discSize = 4
			if !memory.WriteUint32Le(offset, uint32(caseIdx)) {
				return fmt.Errorf("writeValTyped: failed to write variant disc")
			}
		}
		c := td.Variant.Cases[caseIdx]
		if c.ValType == nil || payload == nil {
			return nil
		}
		// Per spec: ptr += disc_size; ptr = align_to(ptr, max_case_alignment).
		maxCaseAlign := uint32(1)
		for _, cc := range td.Variant.Cases {
			if cc.ValType != nil {
				ca := cabiAlignTypeRef(*cc.ValType, localTypes)
				if ca > maxCaseAlign {
					maxCaseAlign = ca
				}
			}
		}
		payloadOffset := alignTo(offset+discSize, maxCaseAlign)
		return writeValTyped(ctx, memory, reallocFunc, payloadOffset, *payload, *c.ValType, localTypes)
	}
	return fmt.Errorf("writeValTyped: unsupported type for value %s", val.Kind())
}

// writeValPrimitive writes a primitive value to memory based on the
// canonical ABI primitive opcode (0x73-0x7f).
func writeValPrimitive(memory api.Memory, offset uint32, val Val, primitive byte) error {
	switch primitive {
	case 0x7f: // bool
		var b byte
		if val.Bool() {
		...
		if !memory.WriteByteAt(offset, b) {
			return fmt.Errorf("writeValPrimitive: failed to write bool")
		}
	case 0x7e: // s8
		if !memory.WriteByteAt(offset, byte(val.S8())) {
			return fmt.Errorf("writeValPrimitive: failed to write s8")
		}
	case 0x7d: // u8
		if !memory.WriteByteAt(offset, val.U8()) {
			return fmt.Errorf("writeValPrimitive: failed to write u8")
		}
	case 0x7c: // s16
		if !memory.WriteUint16Le(offset, uint16(val.S16())) {
			return fmt.Errorf("writeValPrimitive: failed to write s16")
		}
	case 0x7b: // u16
		if !memory.WriteUint16Le(offset, val.U16()) {
			return fmt.Errorf("writeValPrimitive: failed to write u16")
		}
	case 0x7a: // s32
		if !memory.WriteUint32Le(offset, uint32(val.S32())) {
			return fmt.Errorf("writeValPrimitive: failed to write s32")
		}
	case 0x79: // u32
		if !memory.WriteUint32Le(offset, val.U32()) {
			return fmt.Errorf("writeValPrimitive: failed to write u32")
		}
	case 0x78: // s64
		if !memory.WriteUint64Le(offset, uint64(val.S64())) {
			return fmt.Errorf("writeValPrimitive: failed to write s64")
		}
	case 0x77: // u64
		if !memory.WriteUint64Le(offset, val.U64()) {
			return fmt.Errorf("writeValPrimitive: failed to write u64")
		}
	case 0x76: // f32
		if !memory.WriteUint32Le(offset, math.Float32bits(val.F32())) {
			return fmt.Errorf("writeValPrimitive: failed to write f32")
		}
	case 0x75: // f64
		if !memory.WriteUint64Le(offset, math.Float64bits(val.F64())) {
			return fmt.Errorf("writeValPrimitive: failed to write f64")
		}
	case 0x74: // char
		if !memory.WriteUint32Le(offset, uint32(val.Char())) {
			return fmt.Errorf("writeValPrimitive: failed to write char")
		}
	case 0x73: // string: (ptr, len) — caller must have already allocated and written
		// String storage is special: it requires realloc, so it's handled
		// by writeStringTyped at a higher level rather than here. We treat
		// this as an error to ensure callers go through the right path.
		return fmt.Errorf("writeValPrimitive: string must be written via writeStringTyped")
	default:
		return fmt.Errorf("writeValPrimitive: unsupported primitive 0x%x", primitive)
	}
	return nil
}

// writeStringTyped allocates memory for the string via realloc, copies the
// UTF-8 bytes, and writes (ptr, len) at the given offset. This implements
// the canonical ABI store_string_copy for the utf8 -> utf8 case.
func writeStringTyped(ctx context.Context, memory api.Memory, reallocFunc api.Function, offset uint32, s string) error {
	bytes := []byte(s)
	strLen := uint32(len(bytes))
	var ptr uint32
	if strLen > 0 {
		if reallocFunc == nil {
			return fmt.Errorf("writeStringTyped: requires realloc function")
		}
		// realloc(old_ptr, old_size, align, new_size) -> new_ptr
		// UTF-8 strings have alignment 1.
		results, err := reallocFunc.Call(ctx, 0, 0, 1, uint64(strLen))
		if err != nil {
			return fmt.Errorf("writeStringTyped: realloc failed: %w", err)
		}
		ptr = uint32(results[0])
		if !memory.Write(ptr, bytes) {
			return fmt.Errorf("writeStringTyped: failed to write %d bytes at ptr %d", strLen, ptr)
		}
	}
	if !memory.WriteUint32Le(offset, ptr) {
		return fmt.Errorf("writeStringTyped: failed to write string ptr")
	}
	if !memory.WriteUint32Le(offset+4, strLen) {
		return fmt.Errorf("writeStringTyped: failed to write string len")
	}
	return nil
}
...
//
// When the funcType has fully resolved type info for a result, this function
// delegates to writeValTyped which faithfully implements the canonical ABI
// store(cx, v, t, ptr) function. The legacy untyped switch below is kept as a
// fallback for callers that pass funcType==nil or NamedValType entries with
// only primitive type info.
func writeResultsToMemory(ctx context.Context, memory api.Memory, reallocFunc api.Function, retptr uint32, results []Val, funcType *FuncType) error {
	...
	offset := retptr
	for i, result := range results {
		// Type-aware fast path: when we know the result type, use the
		// canonical-ABI-conformant typed writer. This is the only path
		// that correctly handles compound result types like
		//   list<tuple<own<T>, string>>
		//   result<own<T>, error-code>
		//   tuple<...>
		// because it computes alignment and element sizes recursively
		// per the spec.
		if funcType != nil && i < len(funcType.Results) {
			rDef := funcType.Results[i]
			localTypes := rDef.LocalTypes
			if localTypes == nil && rDef.ResolvedType != nil {
				localTypes = rDef.ResolvedType.SourceLocalTypes
			}
			if canWriteValTyped(rDef.ValType, localTypes) {
				// Per the spec, multi-result returns are stored as a
				// despecialized record of result types: each result
				// must be aligned to alignment(t) before its bytes are
				// written.
				ra := cabiAlignTypeRef(rDef.ValType, localTypes)
				offset = alignTo(offset, ra)
				if err := writeValTyped(ctx, memory, reallocFunc, offset, result, rDef.ValType, localTypes); err != nil {
					return fmt.Errorf("result[%d]: %w", i, err)
				}
				offset += cabiSizeTypeRef(rDef.ValType, localTypes)
				continue
			}
		}
		// Untyped fallback path: used when funcType is nil or carries no
		// resolvable type info for this result. The existing legacy switch
		// handles primitive cases. New code should always go through the
		// typed fast path above.
		if funcType != nil && i >= len(funcType.Results) {
			break
		}
		...
		case ValKindOwn, ValKindBorrow:
			...
			offset += 4
		case ValKindList:
			// Untyped fallback: lists of primitive elements only. The
			// type-aware fast path at the top of writeResultsToMemory
			// handles all compound list types correctly.
			list := result.List()
			listLen := uint32(len(list))
		...
		case ValKindEnum:
			...
			if i < len(funcType.Results) && funcType.Results[i].ResolvedType != nil && funcType.Results[i].ResolvedType.Enum != nil {
				...
				disc := uint32(0)
				for j, name := range funcType.Results[i].ResolvedType.Enum.Names {
					if name == caseName {
						disc = uint32(j)
						break
					}
		...
		case ValKindFlags:
			...
			if i < len(funcType.Results) && funcType.Results[i].ResolvedType != nil && funcType.Results[i].ResolvedType.Flags != nil {
				flags := result.Flags()
				var bits uint32
				for j, name := range funcType.Results[i].ResolvedType.Flags.Names {
					if flags[name] {
						bits |= 1 << uint(j)
					}
				}
				memory.WriteUint32Le(offset, bits)
			} else {
				memory.WriteUint32Le(offset, 0)
			}
			offset += 4
		case ValKindRecord:
			...
			if i < len(funcType.Results) && funcType.Results[i].ResolvedType != nil && funcType.Results[i].ResolvedType.Record != nil {
				recordDef := funcType.Results[i].ResolvedType.Record
				localTypes := funcType.Results[i].ResolvedType.SourceLocalTypes
				var err error
				offset, err = writeRecordToMemory(ctx, memory, reallocFunc, offset, rec, recordDef, localTypes)
				if err != nil {
...
func writeRecordToMemory(ctx context.Context, memory api.Memory, reallocFunc api.Function, offset uint32, rec map[string]Val, recordDef *RecordTypeDef, localTypes map[uint32]*TypeDef) (uint32, error) {
	for _, field := range recordDef.Fields {
		...
		var err error
		offset, err = writeValToMemory(ctx, memory, reallocFunc, offset, val, field.ValType, localTypes)
		if err != nil {
			return offset, fmt.Errorf("field %s: %w", field.Name, err)
		}
	}
	return offset, nil
}
...
func writeValToMemory(ctx context.Context, memory api.Memory, reallocFunc api.Function, offset uint32, val Val, typeRef ValTypeRef, localTypes map[uint32]*TypeDef) (uint32, error) {
	...
	case ValKindEnum:
		...
		disc := uint32(0)
		if resolved != nil && resolved.Enum != nil {
			caseName := val.Enum()
			for i, name := range resolved.Enum.Names {
				if name == caseName {
					disc = uint32(i)
					break
				}
	...
	case ValKindOption:
		...
		if payload == nil {
			memory.WriteByteAt(offset, 0)
			offset += 4 // align
			// Write zero bytes for inner type
			innerSize := fieldSizeForType(ValTypeRef{}, localTypes)
			if !typeRef.IsPrimitive && localTypes != nil {
				if td := localTypes[typeRef.TypeIdx]; td != nil && td.Option != nil {
					innerSize = fieldSizeForType(td.Option.InnerType, localTypes)
				}
			}
			return offset + innerSize, nil
		}
		memory.WriteByteAt(offset, 1)
		offset += 4 // align
		var innerTypeRef ValTypeRef
		if !typeRef.IsPrimitive && localTypes != nil {
			if td := localTypes[typeRef.TypeIdx]; td != nil && td.Option != nil {
				innerTypeRef = td.Option.InnerType
			}
		}
	...
	case ValKindList:
		...
		var elemSize uint32 = 1
		if !typeRef.IsPrimitive && localTypes != nil {
			if td := localTypes[typeRef.TypeIdx]; td != nil && td.List != nil {
				elemSize = fieldSizeForType(td.List.ElementType, localTypes)
			}
		} else if len(list) > 0 {
			elemSize = elementSizeForKind(list[0].Kind())
		}
		...
		listSize := listLen * elemSize
		var ptr uint32
		if listSize > 0 && reallocFunc != nil {
			allocResults, err := reallocFunc.Call(ctx, 0, 0, uint64(elemSize), uint64(listSize))
			if err != nil {
				return offset, fmt.Errorf("realloc for list failed: %w", err)
			}
			ptr = uint32(allocResults[0])
			for j, elem := range list {
				writeListElement(memory, ptr+uint32(j)*elemSize, elem)
			}
		}
		memory.WriteUint32Le(offset, ptr)
		offset += 4
	...
	case ValKindString:
		...
		strLen := uint32(len(str))
		var ptr uint32
		if strLen > 0 && reallocFunc != nil {
			allocResults, err := reallocFunc.Call(ctx, 0, 0, 1, uint64(strLen))
			if err != nil {
				return offset, fmt.Errorf("realloc for string failed: %w", err)
			}
			ptr = uint32(allocResults[0])
			memory.Write(ptr, []byte(str))
		}
	...
	case ValKindRecord:
		...
		if resolved != nil && resolved.Record != nil {
			return writeRecordToMemory(ctx, memory, reallocFunc, offset, rec, resolved.Record, localTypes)
		}
		return offset, nil
	default:
		memory.WriteUint32Le(offset, 0)
...
// fieldSizeForType returns the byte size of a type when stored in linear memory.
func fieldSizeForType(ref ValTypeRef, localTypes map[uint32]*TypeDef) uint32 {
	if ref.IsPrimitive {
		switch ref.Primitive {
		case 0x7f: // bool
			return 1
		case 0x7e, 0x7d: // s8, u8
			return 1
		case 0x7c, 0x7b: // s16, u16
			return 2
	...
	if td.Option != nil {
		return 4 + fieldSizeForType(td.Option.InnerType, localTypes)
	}
	if td.Record != nil {
		total := uint32(0)
		for _, f := range td.Record.Fields {
			total += fieldSizeForType(f.ValType, localTypes)
		}
		return total
	}
	return 4
...
// flattenVariantType flattens a variant type.
// Variant is discriminant + max(payload types across all cases), joined by width.
func flattenVariantType(v types.Variant) []api.ValueType {
	// Discriminant
	result := []api.ValueType{api.ValueTypeI32}

	// Find max payload across all cases
	var maxFlat []api.ValueType
	for _, c := range v.Cases {
		if c.Type == nil {
			continue
		}
		caseFlat := flattenValType(c.Type)
		if len(caseFlat) > len(maxFlat) {
			// Extend maxFlat with zero-values
			for len(maxFlat) < len(caseFlat) {
				maxFlat = append(maxFlat, 0)
			}
		}
		for i, t := range caseFlat {
			if i < len(maxFlat) {
				if isWiderValueType(t, maxFlat[i]) {
					maxFlat[i] = t
				}
			}
		}
